...
public class MyListIteratorPublicTester {
    ...
    public void testAddFirstIndex() {
        listLen1Iter.next();
        listLen1Iter.remove();
        listLen1Iter.add("hello");
        assertEquals("Valid add left", "hello",
                listLen1Iter.left.getElement());
        assertEquals("Valid add right", null,
                listLen1Iter.right.getElement());
        assertEquals("Index change after add", 0, listLen1Iter.idx);
        assertFalse("Cannot remove immediately after add",
                listLen1Iter.canRemoveOrSet);
    }
}
